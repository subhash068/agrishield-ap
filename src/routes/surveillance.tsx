...
import {
  ...
  CartesianGrid, BarChart, Bar, RadarChart, Radar, PolarGrid, PolarAngleAxis, Cell,
} from "recharts";
...
export const Route = createFileRoute("/surveillance")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Real-time crop surveillance intelligence — KPIs, NDVI trends, district rankings and AI insights." },
    ],
  }),
  ...
  return (
    <div>
      ...
      <div className="px-6 lg:px-10 py-6 space-y-6">
        ...
        <div className="grid lg:grid-cols-3 gap-5">
          <div className="lg:col-span-2 glass rounded-xl p-5">
            <div className="flex items-center justify-between mb-3">
              <div>
                <h3 className="font-semibold flex items-center gap-2"><Activity className="h-4 w-4 text-primary" /> Spectral Indices (30d)</h3>
                <p className="text-xs text-muted-foreground">NDVI / EVI / NDRE — weighted statewide mean</p>
              </div>
            ...
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={spectralTrend}>
                ...
                <Line type="monotone" dataKey="ndre" stroke="oklch(0.82 0.17 80)"  strokeWidth={2}   dot={false} strokeDasharray="4 3" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        ...
        {/* outbreak heatmap (mock grid) + insights */}
        <div className="grid lg:grid-cols-3 gap-5">
          <div className="lg:col-span-2 glass rounded-xl p-5">
            <div className="flex items-center justify-between mb-3">
            ...
            <div className="grid grid-cols-14 gap-1.5" style={{ gridTemplateColumns: "repeat(14, minmax(0, 1fr))" }}>
              {Array.from({ length: 13 * 14 }, (_, i) => {
                const v = Math.random();
                const bg = v > 0.85 ? "bg-destructive/70" : v > 0.6 ? "bg-warning/60" : v > 0.3 ? "bg-success/40" : "bg-muted/40";
                return <div key={i} className={`aspect-square rounded-sm ${bg} hover:scale-110 transition`} title={`Mandal cell ${i}`} />;
              })}
            </div>
            <p className="mt-3 text-xs text-muted-foreground">Rows = districts (13) · Columns = days (14) · Cell intensity = outbreak severity</p>
          </div>
            ...
            <div className="space-y-2.5">
              {alerts.slice(0, 5).map(a => (
                <div key={a.id} className="rounded-lg border border-border/60 bg-muted/20 p-3">
                  ...
                  <p className="mt-1 text-[11px] text-muted-foreground">{a.district} · {a.crop} · {a.time}</p>
                </div>
              ))}
            </div>
          </div>
