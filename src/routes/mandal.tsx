...
export const Route = createFileRoute("/mandal")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Government-level surveillance dashboard: mandal-wise outbreaks, ranks and RSK monitoring." },
    ],
  }),
    ...
    <div>
      ...
      <div className="px-6 lg:px-10 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            { label: "Mandals Online", value: "679 / 679", sub: "APRTGS sync OK", color: "text-success" },
            { label: "RSKs Reporting", value: "10,634", sub: "Real-time", color: "text-primary" },
            { label: "Outbreaks Active", value: "318", sub: "Across 47 mandals", color: "text-warning" },
        ...
        <div className="grid lg:grid-cols-[1.4fr_1fr] gap-5">
          <div className="glass rounded-xl p-5">
            ...
            <p className="text-xs text-muted-foreground mb-3">Real-time ranking · live AI surveillance</p>
            <ResponsiveContainer width="100%" height={360}>
              <BarChart data={mandalRanks} layout="vertical" margin={{ left: 60 }}>
                ...
                <YAxis type="category" dataKey="mandal" tick={{ fontSize: 10, fill: "oklch(0.9 0.02 180)" }} width={80} />
                ...
                <Bar dataKey="outbreaks" radius={[0, 6, 6, 0]}>
                  {mandalRanks.map((m, i) => (
                    <Cell key={i} fill={m.outbreaks > 60 ? "oklch(0.68 0.22 25)" : m.outbreaks > 30 ? "oklch(0.82 0.17 80)" : "oklch(0.78 0.19 145)"} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ...
          <div className="glass rounded-xl p-5">
            ...
            <div className="space-y-2.5">
              {[
                ...
                <div key={r.name} className="flex items-center justify-between rounded-lg border border-border/60 bg-muted/20 p-3">
                  <div>
                    <p className="text-sm font-semibold">{r.name}</p>
                    <p className="text-[11px] text-muted-foreground">Officer · {r.officer} · {r.visits} visits today</p>
                  </div>
                  <Badge variant="outline" className={r.status === "Active" ? "border-success/40 text-success bg-success/10" : "border-warning/40 text-warning bg-warning/10"}>{r.status}</Badge>
                </div>
              ))}
            </div>
          </div>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="text-left text-muted-foreground border-b border-border/60">
                    <th className="py-2">#</th><th>District</th><th className="text-right">Health</th><th className="text-right">Alerts</th><th className="text-right">Trend</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border/40">
                  {districtRankings.slice(0, 10).map(d => (
                    <tr key={d.district}>
                      ...
                      <td className="text-right tabular-nums">{d.alerts.toLocaleString("en-IN")}</td>
                      <td className="text-right">
                        {d.riskIndex > 50 ?
                          <span className="inline-flex items-center gap-0.5 text-destructive"><TrendingUp className="h-3 w-3" />{d.riskIndex}</span> :
                          <span className="inline-flex items-center gap-0.5 text-success"><TrendingDown className="h-3 w-3" />{d.riskIndex}</span>
                        }
                      </td>
                    </tr>
                  ))}
