...
import { getDistricts, getWeatherHistory, getWeatherLiveSummary, getWeatherProjection2027 } from "@/lib/api";
...
export const Route = createFileRoute("/weather")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Historical weather from 2024 to today and a 2027 projection based on that history." },
    ],
  }),
...
function WeatherPage() {
  ...
  const { data: liveSummary } = useQuery({ queryKey: ["weather-live"], queryFn: getWeatherLiveSummary });
  ...
  const selectedDistrictIndex = selectedDistrict === "all" ? 0 : Math.max(availableDistricts.indexOf(selectedDistrict), 0);
  const selectedDistrictPoint = weatherData.length ? weatherData[selectedDistrictIndex % weatherData.length] : null;
  ...
  const rainHeat = availableDistricts.map((district, index) => {
    const baseRain = weatherData.length ? Math.round(weatherData.reduce((sum, point) => sum + point.rainfall, 0)) : 0;
    const rain = Math.max(0, baseRain - index * 3 + Math.round((weatherData[index % Math.max(weatherData.length, 1)]?.rainfall ?? 0) * 2));
    const deficit = weatherData.length ? Math.round(weatherData[index % weatherData.length]?.drought ?? 0) - 50 : 0;
    return { district, rain, deficit };
  });

  const focusedTemp = selectedDayPoint?.temp ?? selectedDistrictPoint?.temp ?? liveSummary?.temperature ?? 0;
  const focusedRain = selectedDayPoint?.rainfall ?? selectedDistrictPoint?.rainfall ?? liveSummary?.rainfall_24h ?? 0;
  const focusedHumidity = selectedDayPoint?.humidity ?? selectedDistrictPoint?.humidity ?? liveSummary?.humidity ?? 0;
  ...
  const avgHumidity = weatherData.length ? Math.round(weatherData.reduce((sum, point) => sum + point.humidity, 0) / weatherData.length) : liveSummary?.humidity ?? 0;
      ...
      <div className="px-6 lg:px-10 py-6 space-y-6">
        <div className="glass rounded-xl p-4 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Switch between observed history and 2027 projection.</p>
          </div>
          <div className="flex w-full flex-col gap-2 md:w-auto md:flex-row">
            ...
            <div className="w-full md:w-[260px]">
              <Select value={selectedMode} onValueChange={(value) => setSelectedMode(value as WeatherMode)}>
                <SelectTrigger>
        ...
        <div className="glass rounded-xl p-4 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Switch the weather panels to a specific district.</p>
          </div>
          <div className="w-full md:w-[260px]">
        ...
        <div className="glass rounded-xl p-4 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Pick a specific day from the selected weather dataset.</p>
          </div>
          <div className="w-full md:w-[260px]">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            { icon: Thermometer, label: "Temperature", value: `${focusedTemp.toFixed(1)}°C`, trend: selectedDayPoint ? selectedDayPoint.day : activeModeLabel, color: "text-warning" },
            { icon: CloudRain, label: "Rainfall", value: `${focusedRain.toFixed(0)} mm`, trend: selectedDayPoint ? `${selectedDayPoint.day} selected` : `${totalRain} mm total`, color: "text-info" },
            { icon: Droplets, label: "Humidity", value: `${focusedHumidity}%`, trend: selectedMode === "projection" ? "Projected" : "Observed", color: "text-accent" },
            { icon: Wind, label: "Wind Speed", value: `${windSpeed} km/h`, trend: liveSummary ? liveSummary.location : "Live", color: "text-primary" },
          ].map((c) => (
        ...
        <div className="grid lg:grid-cols-2 gap-5">
          <div className="glass rounded-xl p-5">
            ...
            <p className="text-xs text-muted-foreground mb-3">Daily rainfall for the active dataset</p>
