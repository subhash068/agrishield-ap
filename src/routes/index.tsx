  ...
  head: () => ({
    meta: [
      { title: "AgriShield AP — Command Center" },
      {
        name: "description",
        content:
          "Real-time AI agriculture intelligence for Andhra Pradesh: 4.2M+ farmers, 1.9M+ parcels, satellite-grade crop surveillance.",
      },
      { property: "og:title", content: "AgriShield AP — Command Center" },
      {
        property: "og:description",
        content: "Real-time AI agriculture intelligence for Andhra Pradesh.",
      },
    ],
  }),
  component: HomePage,
...
function HomePage() {
  ...
  const filteredDistrictRankings =
    selectedDistrict === "all"
      ? districtRankings
      : districtRankings.filter((district) => district.district === selectedDistrict);
              ...
              >
                <Button asChild size="lg" className="gap-2 glow-primary">
                  <Link to="/surveillance">
                    <Activity className="h-4 w-4" /> Open Surveillance Dashboard
                  </Link>
                </Button>
                <Button
                  asChild
                ...
                >
                  <Link to="/satellite">
                    <Satellite className="h-4 w-4" /> Launch GIS Console
                  </Link>
                </Button>
              </motion.div>
            </div>
              ...
              <div className="relative h-64 w-64">
                {[1, 2, 3].map((r) => (
                  <div
                    ...
                    style={{
                      transform: `scale(${0.5 + r * 0.2})`,
                      animation: `spin ${20 + r * 10}s linear infinite`,
                    }}
                  />
                ))}
                ...
                <div
                  className="absolute bottom-6 left-4 h-3 w-3 rounded-full bg-warning pulse-ring"
          ...
          <div className="mt-8 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {heroStats.map((s, i) => (
              ...
              >
                ...
                <p className="mt-1 text-xl lg:text-2xl font-bold tabular-nums">
      ...
      <section className="px-6 lg:px-10 py-8 space-y-8">
        ...
        <div className="grid lg:grid-cols-[1.2fr_0.8fr] gap-5">
          ...
          >
            ...
            <p className="mt-3 text-sm lg:text-base text-muted-foreground max-w-3xl">
              This platform is designed to help Andhra Pradesh move from reactive crop recovery to
              proactive crop protection. It combines remote sensing, AI, and field-friendly
              advisories so stress can be identified before losses become severe.
            </p>
          ...
          >
            ...
            <div className="mt-4 rounded-lg border border-primary/30 bg-primary/10 p-4">
              ...
              <p className="mt-2 text-sm leading-6 text-muted-foreground">
                Paddy, cotton, chilli, red gram, and maize are the priority crops for early stress
                detection and advisory delivery.
              </p>
            </div>
          </motion.aside>
        </div>
          ...
          <div className="lg:col-span-2 glass rounded-xl p-5">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Live eligibility & disbursement tracker
                </p>
              </div>
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              {schemes.map((s) => (
                <div
                  key={s.title}
                  className="rounded-lg border border-border/60 bg-muted/20 p-4 hover:border-primary/40 transition"
                >
          ...
          <div className="glass rounded-xl p-5">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-lg font-semibold flex items-center gap-2">
                <AlertTriangle className="h-4 w-4 text-warning" /> Emergency Alerts
              </h2>
        ...
        <div className="grid lg:grid-cols-3 gap-5">
          <div className="lg:col-span-2 glass rounded-xl p-5">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="text-lg font-semibold flex items-center gap-2">
                  <TrendingUp className="h-4 w-4 text-primary" /> 30-Day Crop Health Trends
                </h2>
                <p className="text-xs text-muted-foreground">
                  NDVI · EVI · NDRE — statewide weighted average
                </p>
              </div>
            </div>
        ...
        <div className="glass rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-lg font-semibold flex items-center gap-2">
                <MapPin className="h-4 w-4 text-accent" /> District Health Rankings
              </h2>
              <p className="text-xs text-muted-foreground">
                Live AI-computed crop health index across all districts
              </p>
            </div>
            <Button asChild variant="outline" size="sm" className="gap-1">
              <Link to="/mandal">
                Mandal view <ArrowRight className="h-3 w-3" />
              </Link>
            </Button>
          </div>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={filteredDistrictRankings} layout="vertical" margin={{ left: 80 }}>
              <CartesianGrid
              ...
              <Bar dataKey="healthScore" radius={[0, 6, 6, 0]}>
                {filteredDistrictRankings.map((d, i) => (
                  <Cell
                    key={i}
                    fill={
                      d.healthScore > 80
        ...
        <div className="grid md:grid-cols-3 gap-4">
          {[
            {
              icon: Bug,
              title: "Outbreak Forecast",
              text: "AI predicts 72% probability of cotton bollworm spread in Guntur within 14 days.",
              tone: "warning",
            },
            {
              icon: CloudRain,
              title: "Weather Risk",
              text: "Rainfall deficit at 18% across Rayalaseema. Drought advisory recommended.",
              tone: "info",
            },
            {
              icon: Sparkles,
              title: "Yield Boost",
              text: "Optimal nitrogen schedule could lift paddy yield by 6.4% in Krishna delta.",
              tone: "primary",
            },
          ].map((c, i) => (
            <motion.div
            ...
            >
              <div
                className={`h-10 w-10 rounded-lg grid place-items-center mb-3 ${
                  c.tone === "warning"
