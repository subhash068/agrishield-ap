      ...
      <div className="px-6 lg:px-10 py-6 grid lg:grid-cols-[1.2fr_1fr] gap-6">
        <div className="glass rounded-xl p-6">
              ...
              <div className="mt-4 flex justify-center gap-2 flex-wrap">
                <label className="inline-flex">
                  ...
                  <Button size="sm" variant="outline" className="gap-1.5" asChild>
                    <span><Camera className="h-3.5 w-3.5" /> Choose Image</span>
                  </Button>
                </label>
          ...
          <div className="mt-4 grid grid-cols-3 gap-3 text-xs">
            ...
            <div className="glass rounded-lg p-3"><div className="flex items-center gap-1.5 text-muted-foreground"><Clock className="h-3 w-3" /> Timestamp</div><div className="mt-1 font-medium">{new Date().toLocaleTimeString()}</div></div>
            <div className="glass rounded-lg p-3"><div className="flex items-center gap-1.5 text-muted-foreground"><CheckCircle2 className="h-3 w-3" /> Model</div><div className="mt-1 font-medium">HF Plant Classifier</div></div>
          </div>
        </div>
            ...
            <>
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass rounded-xl p-5">
                <div className="flex items-start justify-between">
                  ...
                  <Badge className={
                    ...
                    result.severity === "Low" ? "bg-success/20 text-success border-success/40" :
              ...
              <div className="glass rounded-xl p-5">
                ...
                <ul className="mt-2 space-y-1.5 text-xs text-muted-foreground">
                  {result.top_k.map((p) => (
                    <li key={p.label}>{p.label} - {(p.score * 100).toFixed(2)}%</li>
                  ))}
                </ul>
              </div>
